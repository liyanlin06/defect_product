...
base_model = tf.keras.applications.resnet.ResNet101(layers=tf.keras.layers, include_top=False, weights='imagenet', input_tensor=inputs)
## 冻结参数
...
# base_model = tf.keras.applications.resnet.ResNet101(include_top=False, input_tensor=inputs)
# x = tf.keras.layers.GlobalAveragePooling2D(name='average_pool')(base_model.output)
# fla = tf.keras.layers.Flatten()(base_model.output)
# fc1 = tf.keras.layers.Dense(1024, activation='relu')(fla)
# drop1 = tf.keras.layers.Dropout(rate=0.2)(fc1)
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath='train_process/model/ResNet101_ft.h5',
