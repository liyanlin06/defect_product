...
# base_model = tf.keras.applications.inception_resnet_v2.InceptionResNetV2(layers = tf.keras.layers, include_top=False, weights = 'imagenet', input_tensor=inputs)
## 冻结参数
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath='train_process/model/VGG19_ft.h5', monitor='val_categorical_accuracy', verbose=1, save_best_only=True, mode='max')
callback_list = [checkpoint]
model.compile(optimizer=optimizer, loss=loss, metrics=['categorical_accuracy'])


print("-----------开始训练----------")
model.fit(train_data, train_label, epochs = epochs, batch_size = batch_size, validation_data=(test_data, test_label), callbacks=callback_list)
